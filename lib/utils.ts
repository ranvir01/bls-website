 ...
 * colour. This project's type scale uses names it has never seen —
 * `text-body`, `text-caption`, `text-h1` — so it filed them under *colour* and
 * then treated them as conflicting with real colours.
 *
 * The button variants are `bg-leaf-600 text-white` + `min-h-[48px] px-6
 ...
 * surrounding section used — dark slate on a green button, about 3:1. It
 * looked like a design choice rather than a bug, which is why it survived a
 * full visual pass and why the reported symptom was "can't even see it".
 *
 ...
 * fontSize or boxShadow key added to tailwind.config.ts has to be added here
 * too.
 */
const twMerge = extendTailwindMerge({
  extend: {
    classGroups: {
      ...
      shadow: [{ shadow: ['subtle', 'card', 'lifted', 'header'] }],
